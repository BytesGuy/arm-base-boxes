...
locals {
  data_source_content = {
    "/preseed.cfg" = templatefile("${abspath(path.root)}/data/preseed.cfg.pkrtpl.hcl", {
...
source "vmware-iso" "debian-11" {
  ...
  boot_command = [
    "c",
    "linux /install.a64/vmlinuz auto-install/enable=true debconf/priority=critical noprompt url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed.cfg --<enter>",
...
build {
  sources = ["sources.vmware-iso.debian-11"]
